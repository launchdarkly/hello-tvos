...
static NSString * _Nonnull const LaunchDarklyJSONErrorDomain = @"LaunchDarkly_tvOS.JSONSerialization.JSONError";
...
/// \param userWrapper The LDUser set with the desired user. If omitted, LDClient retains the previously set user, or default if one was never set. (Optional)
///
/// \param startWaitSeconds A TimeInterval representing how long to wait for flags before returning true in the completion to indicate that it timed out.
///
/// \param completion Closure called when the embedded <code>setOnlineIdentify</code> call completes, subject to throttling delays. Takes a Bool as a parameter that indicates whether the SDK did not come online within startWaitSeconds. (Optional)
///
- (void)startCompleteWhenFlagsReceivedWithConfig:(LDConfig * _Nonnull)configWrapper user:(LDUser * _Nullable)userWrapper startWaitSeconds:(NSTimeInterval)startWaitSeconds completion:(void (^ _Nullable)(BOOL))completion;
