...
/// An NSObject which mimics Swift’s Result type, specifically for the <code>LDContext</code> type.
SWIFT_CLASS("_TtC17LaunchDarkly_tvOS20ContextBuilderResult")
@interface ContextBuilderResult : NSObject
@property (nonatomic, readonly, strong) LDContext * _Nullable success;
@property (nonatomic, readonly, strong) NSError * _Nullable failure;
...
/// <ol>
///   <li>
///     Call <code>[ObjcLDClient startWithConfig: context: completion:]</code> (<code>ObjcLDClient.startWithConfig(_:config:context:completion:)</code>)
///   </li>
/// </ol>
/// <ul>
///   <li>
...
/// When online, the SDK communicates with LaunchDarkly servers for feature flag values and event reporting.
/// When offline, the SDK does not attempt to communicate with LaunchDarkly servers. Client apps can request feature flag values and set/change feature flag observers while offline. The SDK will collect events while offline.
/// The SDK protects itself from multiple rapid calls to <code>setOnline:YES</code> by enforcing an increasing delay (called <em>throttling</em>) each time <code>setOnline:YES</code> is called within a short time. The first time, the call proceeds normally. For each subsequent call the delay is enforced, and if waiting, increased to a maximum delay. When the delay has elapsed, the <code>setOnline:YES</code> will proceed, assuming that the client app has not called <code>setOnline:NO</code> during the delay. Therefore a call to <code>setOnline:YES</code> may not immediately result in the LDClient going online. Client app developers should consider this situation abnormal, and take steps to prevent the client app from making multiple rapid <code>setOnline:YES</code> calls. Calls to <code>setOnline:NO</code> are not throttled. Note that calls to <code>start(config: context: completion:)</code>, and setting the <code>config</code> or <code>context</code> can also call <code>setOnline:YES</code> under certain conditions. After the delay, the SDK resets and the client app can make a susequent call to <code>setOnline:YES</code> without being throttled.
...
/// The LDContext set into the LDClient may affect the set of feature flags returned by the LaunchDarkly server, and ties event tracking to the context. See <code>LDContext</code> for details about what information can be retained.
/// The client app can change the current LDContext by calling this method. Client apps should follow <a href="apple.com/legal/privacy">Apple’s Privacy Policy</a> when collecting user information. When a new context is set, the LDClient goes offline and sets the new context. If the client was online when the new context was set, it goes online again, subject to a throttling delay if in force (see <code>setOnline(_: completion:)</code> for details).
/// \param context The ObjcLDContext set with the desired context.
///
- (void)identifyWithContext:(LDContext * _Nonnull)context;
/// Deprecated identify method which accepts a legacy LDUser instead of an LDContext.
/// This LDUser will be converted into an LDContext, and the context specific version of this method will be called. See <code>identify(context)</code> for details.
- (void)identifyWithUser:(LDUser * _Nonnull)user;
/// The LDContext set into the LDClient may affect the set of feature flags returned by the LaunchDarkly server, and ties event tracking to the context. See <code>LDContext</code> for details about what information can be retained.
/// Normally, the client app should create and set the LDContext and pass that into <code>start(config: context: completion:)</code>.
/// The client app can change the active <code>context</code> by calling identify with a new or updated LDContext. Client apps should follow <a href="apple.com/legal/privacy">Apple’s Privacy Policy</a> when collecting user information. If the client app does not create a LDContext, LDClient creates an anonymous default context, which can affect the feature flags delivered to the LDClient.
/// When a new context is set, the LDClient goes offline and sets the new context. If the client was online when the new context was set, it goes online again, subject to a throttling delay if in force (see <code>setOnline(_: completion:)</code> for details). To change both the <code>config</code> and <code>context</code>, set the LDClient offline, set both properties, then set the LDClient online. A completion may be passed to the identify method to allow a client app to know when fresh flag values for the new context are ready.
/// \param context The ObjcLDContext set with the desired context.
///
...
///
- (void)identifyWithContext:(LDContext * _Nonnull)context completion:(void (^ _Nullable)(void))completion;
/// Deprecated identify method which accepts a legacy LDUser instead of an LDContext.
/// This LDUser will be converted into an LDContext, and the context specific version of this method will be called. See <code>identify(context:completion:)</code> for details.
...
- (void)flush;
/// Starts the LDClient using the passed in <code>config</code> & <code>context</code>. Call this before requesting feature flag values. The LDClient will not go online until you call this method.
/// Starting the LDClient means setting the <code>config</code> & <code>context</code>, setting the client online if <code>config.startOnline</code> is true (the default setting), and starting event recording. The client app must start the LDClient before it will report feature flag values. If a client does not call <code>start</code>, no methods will work.
/// If the <code>start</code> call omits the <code>context</code>, the LDClient uses the default <code>context</code> if it was never set.
...
/// Subsequent calls to this method cause the LDClient to return. Normally there should only be one call to start. To change <code>context</code>, use <code>identify</code>.
/// \param configuration The LDConfig that contains the desired configuration. (Required)
///
/// \param context The LDContext set with the desired context. If omitted, LDClient sets a default context. (Optional)
///
/// \param completion Closure called when the embedded <code>setOnline</code> call completes. (Optional)
///
+ (void)startWithConfiguration:(LDConfig * _Nonnull)configuration context:(LDContext * _Nonnull)context completion:(void (^ _Nullable)(void))completion;
/// Deprecated start method which accepts a legacy LDUser instead of an LDContext.
/// This LDUser will be converted into an LDContext, and the context specific version of this method will be called. See <code>start(configuration:context:completion:)</code> for details.
...
///
+ (void)startWithConfiguration:(LDConfig * _Nonnull)configuration context:(LDContext * _Nonnull)context startWaitSeconds:(NSTimeInterval)startWaitSeconds completion:(void (^ _Nullable)(BOOL))completion;
/// Deprecated start method which accepts a legacy LDUser instead of an LDContext.
/// This LDUser will be converted into an LDContext, and the context specific version of this method will be called. See <code>start(configuration:context:startWaitSeconds:completion:)</code> for details.
...
/// See Also: <code>privateContextAttributes</code> and <code>LDContext.privateAttributes</code> (<code>ObjcLDContext.privateAttributes</code>)
@property (nonatomic) BOOL allContextAttributesPrivate;
/// Context attributes and top level custom dictionary keys to treat as private for event reporting for all contexts.
/// The SDK will not include private attribute values in analytics events, but private attribute names will be sent.
/// To set private context attributes for a specific context, see <code>LDContext.privateAttributes</code> (<code>ObjcLDContext.privateAttributes</code>). (Default: <code>[]</code>)
/// See Also: <code>allContextAttributesPrivate</code> and <code>LDContext.privateAttributes</code> (<code>ObjcLDContext.privateAttributes</code>).
@property (nonatomic, copy) NSArray<NSString *> * _Nonnull privateContextAttributes;
...
@property (nonatomic) NSInteger maxCachedContexts;
...
/// events.
/// To create an LDContext of a single kind, such as a user, you may use <code>LDContextBuilder</code>.
/// To create an LDContext with multiple kinds, use <code>LDMultiContextBuilder</code>.
SWIFT_CLASS_NAMED("ObjcLDContext")
@interface LDContext : NSObject
...
- (NSString * _Nonnull)fullyQualifiedKey SWIFT_WARN_UNUSED_RESULT;
///
/// returns:
/// true if the <code>LDContext</code> is a multi-context; false otherwise.
- (BOOL)isMulti SWIFT_WARN_UNUSED_RESULT;
/// / - Returns: A hash mapping a context’s kind to its key.
- (NSDictionary<NSString *, NSString *> * _Nonnull)contextKeys SWIFT_WARN_UNUSED_RESULT;
/// Looks up the value of any attribute of the <code>LDContext</code>, or a value contained within an
/// attribute, based on a <code>Reference</code>. This includes only attributes that are addressable in evaluations.
/// This implements the same behavior that the SDK uses to resolve attribute references during a flag
/// evaluation. In a context, the <code>Reference</code> can represent a simple attribute name– either a
/// built-in one like “name” or “key”, or a custom attribute that was set by <code>LDContextBuilder.trySetValue(...)</code>–
/// or, it can be a slash-delimited path using a JSON-Pointer-like syntax. See <code>Reference</code> for more details.
/// For a multi-context, the only supported attribute name is “kind”.
/// If the value is found, the return value is the attribute value, using the type <code>LDValue</code> to
/// represent a value of any JSON type.
/// If there is no such attribute, or if the <code>Reference</code> is invalid, the return value is nil.
- (LDValue * _Nullable)getValueWithReference:(Reference * _Nonnull)reference SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end


/// Contains methods for building a single kind <code>LDContext</code> with a specified key, defaulting to kind
/// “user”.
/// You may use these methods to set additional attributes and/or change the kind before calling
/// <code>LDContextBuilder.build()</code>. If you do not change any values, the defaults for the <code>LDContext</code> are that its
/// kind is “user”, its key is set to whatever value you passed to <code>LDContextBuilder.init(key:)</code>, its anonymous attribute
/// is false, and it has no values for any other attributes.
/// To define a multi-context, see <code>LDMultiContextBuilder</code>.
SWIFT_CLASS_NAMED("ObjcLDContextBuilder")
@interface LDContextBuilder : NSObject
...
/// If <code>LDContextBuilder.key</code> is called, a key will no longer be generated and the anonymous status will match the
/// value provided by <code>LDContextBuilder.anonymous</code> or false by default.
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
/// Create a new LDContextBuilder with the provided <code>key</code>.
...
///   </li>
/// </ul>
/// If the value is invalid, you will receive an error when <code>LDContextBuilder.build()</code> is called.
- (void)kindWithKind:(NSString * _Nonnull)kind;
...
- (void)keyWithKey:(NSString * _Nonnull)key;
...
///   </li>
/// </ul>
- (void)nameWithName:(NSString * _Nonnull)name;
...
- (void)anonymousWithAnonymous:(BOOL)anonymous;
...
/// This action only affects analytics events that involve this particular <code>LDContext</code>. To mark some (or all)
...
///
/// \endcodeThe attributes “kind”, “key”, and “anonymous” cannot be made private.
...
/// use whatever value (if any) you have set for that name with <code>trySetValue(...)</code>.
/// <h1>Designating an entire attribute as private</h1>
/// If the parameter is an attribute name such as “email” that does not start with a ‘/’ character, the
/// entire attribute is private.
/// <h1>Designating a property within a JSON object as private</h1>
...
- (void)addPrivateAttributeWithReference:(Reference * _Nonnull)reference;
/// Remove any reference provided through <code>addPrivateAttribute(_:)</code>. If the reference was
/// added more than once, this method will remove all instances of it.
- (void)removePrivateAttributeWithReference:(Reference * _Nonnull)reference;
/// Sets the value of any attribute for the Context except for private attributes.
/// This method uses the <code>LDValue</code> type to represent a value of any JSON type: null,
...
/// <ul>
///   <li>
///     “kind”, “key”: Must be a string. See <code>LDContextBuilder.kind(_:)</code> and <code>LDContextBuilder.key(_:)</code>.
///   </li>
///   <li>
///     “name”: Must be a string or null. See <code>LDContextBuilder.name(_:)</code>.
///   </li>
///   <li>
///     “anonymous”: Must be a boolean. See <code>LDContextBuilder.anonymous(_:)</code>.
///   </li>
/// </ul>
/// Values that are JSON arrays or objects have special behavior when referenced in
/// flag/segment rules.
/// A value of <code>LDValue.null</code> is equivalent to removing any current non-default value
...
- (BOOL)trySetValueWithName:(NSString * _Nonnull)name value:(LDValue * _Nonnull)value;
...
- (ContextBuilderResult * _Nonnull)build SWIFT_WARN_UNUSED_RESULT;
@end
...
/// own nested LDContext. To define a single-kind context, use <code>LDContextBuilder</code> instead.
/// Obtain an instance of LDMultiContextBuilder by calling <code>LDMultiContextBuilder.init()</code>; then, call
/// <code>LDMultiContextBuilder.addContext(_:)</code> to specify the nested LDContext for each kind.
/// LDMultiContextBuilder setters return a reference the same builder, so they can be chained
/// together.
SWIFT_CLASS_NAMED("ObjcLDMultiContextBuilder")
@interface LDMultiContextBuilder : NSObject
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
...
/// you call <code>LDMultiContextBuilder.build()</code>.
- (void)addContextWithContext:(LDContext * _Nonnull)context;
...
/// If only one context kind was added to the builder, <code>build</code> returns a single-kind context rather
/// than a multi-context.
- (ContextBuilderResult * _Nonnull)build SWIFT_WARN_UNUSED_RESULT;
@end


SWIFT_CLASS_NAMED("ObjcLDReference")
...
SWIFT_CLASS_NAMED("ObjcLDReferenceError")
@interface ReferenceError : NSObject
...
/// The usage of LDUser is no longer recommended and is retained only to ease the adoption of the <code>LDContext</code> class. New
/// code using this SDK should make use of the <code>LDContextBuilder</code> to construct an equivalent <code>Kind.user</code> kind context.
...
- (ContextBuilderResult * _Nonnull)toContext SWIFT_WARN_UNUSED_RESULT;
@end
